...
if ($display -eq 'Y') {
    $inactiveUsers | Select-Object SamAccountName, lastLogonTimestamp | Format-Table -AutoSize
}
...
if ($confirm -eq 'Y') {
    foreach ($user in $inactiveUsers) {
        Disable-ADAccount -Identity $user.SamAccountName -Server $domainController
...
if ($export -eq 'Y') {
    $inactiveUsers | Select-Object SamAccountName, lastLogonTimestamp | Export-Csv -Path "C:\InactiveUsers.csv" -NoTypeInformation
    Write-Host "The list of inactive users has been exported to C:\InactiveUsers.csv"
} 
else {
